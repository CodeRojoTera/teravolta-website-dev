...
must_haves:
  truths:
    ...
    - "AdvocacyFields renders claim-related inputs"
...
import { useLanguage } from '@/components/LanguageProvider';
import { QuoteFormData } from '@/lib/schemas/quote-schema';
import { isInspectionRequired, PropertyType } from '@/lib/schemas/constants';
 ...
 * Per ADVO-11: Shows claim_type, distributor_company, claim_amount, incident_date, damage_description.
 * 
 * Hides: property_size, device_option, connectivity, booking, bills, phases
 */
export function AdvocacyFields({
  ...
  const t = {
    timeline: language === 'es' ? 'Urgencia del Caso' : 'Case Urgency',
    timelinePlaceholder: language === 'es' ? 'ej. Inmediato, 1-2 semanas' : 'e.g., Immediate, 1-2 weeks',
    budget: language === 'es' ? 'Presupuesto para Gestión' : 'Budget for Services',
    budgetPlaceholder: language === 'es' ? 'ej. $1,000 - $5,000' : 'e.g., $1,000 - $5,000',
    description: language === 'es' ? 'Descripción del Problema' : 'Problem Description',
    descriptionPlaceholder: language === 'es' 
      ? 'Describe el problema con tu servicio eléctrico...'
      : 'Describe the issue with your electric service...',
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        {(errors as any).timeline && (
          <span className="text-red-500 text-sm mt-1 block">
            {(errors as any).timeline.message}
          </span>
        )}
      </div>
      ...
      <div>
        ...
        {(errors as any).projectDescription && (
          <span className="text-red-500 text-sm mt-1 block">
            {(errors as any).projectDescription.message}
          </span>
        )}
      </div>
...
- AdvocacyFields: Same core fields + claim-specific fields (optional)
...
Verify AdvocacyFields has claim-specific section with 5 claim fields.
  </verify>
  <done>
- Both files exist at expected paths
- ConsultingFields has 3 required field inputs
- AdvocacyFields has 3 required + 5 optional claim fields
- Both show validation errors from RHF
- NO property_size, device_option, or connectivity in either component
  </done>
</task>
...
3. **Field presence verification:**
   ...
   - AdvocacyFields: timeline, budget, projectDescription, claimType, distributorCompany, claimAmount, incidentDate, damageDescription
</verification>
...
4. ConsultingFields has timeline, budget, projectDescription with required indicators
5. AdvocacyFields has core fields plus claim-specific section
6. All components support React Hook Form register pattern
7. All components display validation errors
8. All components have bilingual label support
</success_criteria>
